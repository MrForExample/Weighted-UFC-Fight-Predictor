...
fighter_dynamic_columns = ['AGE', 'FIGHT_COUNT', 'FIGHT_MINUTE', 'REV_I', 'REV_P', 'DEC_DRAW_%', 'NC_%', 'STANCE_ORT', 'STANCE_SOU', 'STANCE_SWI']
...
target_win_columns += ['DEC_DRAW_%_D_TAR_2', 'NC_%_D_TAR_2']
...
def build_train_fight_data():
    ...
    for _, reformat_row in reverse_reformat_df.iterrows(): 
        ...
        for col, method in zip(['DEC_DRAW_%', 'NC_%'], ['DCD', 'NC']):
            now_col_value = int(fight_win_method == method)
            new_fight_row[col + '_' + str(id_i)] = fighter_build_info[fighter_id][2][col]
            new_fight_row[col + '_' + str(other_id_i)] = fighter_build_info[other_fighter_id][2][col]
            new_fight_row[col + '_D_TAR_2'] = now_col_value
            
            fighter_build_info[fighter_id][2][col] = get_avg_value(now_col_value, 
                                                fighter_build_info[fighter_id][2][col], 
                                                fighter_build_info[fighter_id][4][0])
            fighter_build_info[other_fighter_id][2][col] = get_avg_value(now_col_value, 
                                                fighter_build_info[other_fighter_id][2][col], 
                                                fighter_build_info[other_fighter_id][4][0])
...
def build_and_process_predict_fight_data():
    ...
    if path.exists(fighter_build_info_path):
        ...
        for fp_i in range(all_need_predict_num):
            ...
            for col in ['DEC_DRAW_%', 'NC_%']:
                new_need_predict_row[col + '_' + str(id_i)] = fighter_build_info[fighter_id][2][col]
                new_need_predict_row[col + '_' + str(other_id_i)] = fighter_build_info[other_fighter_id][2][col]
